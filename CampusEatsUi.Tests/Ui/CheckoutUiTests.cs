...
public class CheckoutUiTests : BunitContext
{
    ...
    [Fact]
    public void Pay_Button_RedirectsTo_Stripe_Checkout()
    {
        // Arrange
        var userId = Guid.NewGuid();
        var menuItemId = Guid.NewGuid();
        ...
        _orderServiceMock.Setup(x => x.CreateOrderAsync(userId, _cartState.CartItems, "Generated by the UI"))
            .ReturnsAsync(new Orders(orderId, userId, new List<OrderItems>(), string.Empty, 0, DateTime.Now, null, string.Empty));

        var paymentId = Guid.NewGuid();
        var checkoutUrl = "https://example.com/checkout";
        _paymentServiceMock.Setup(x => x.CreateCheckoutSessionAsync(orderId, userId, null, null))
            .ReturnsAsync(new PaymentResponse(paymentId, "sess_123", checkoutUrl, "pending", 1000, "usd"));
        ...
        // Assert
        _orderServiceMock.Verify(x => x.CreateOrderAsync(userId, _cartState.CartItems, "Generated by the UI"), Times.Once);
        _paymentServiceMock.Verify(x => x.CreateCheckoutSessionAsync(orderId, userId, null, null), Times.Once);
        Assert.Equal(checkoutUrl, navMan.Uri);
    }
}